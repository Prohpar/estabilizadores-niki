...
st.markdown("Dimensionamiento de capacidad aparente (kVA) con catálogo de motores industriales de inducción según norma NEC (NFPA 70).")

# --- TABLAS DE CORRIENTE NOMINAL DE MOTORES DE INDUCCIÓN (NEC TABLAS 430.248 / 430.250) ---
MOTORES_NEC = {
    "Monofásico 120V": {
        "0.5 HP": 9.8,
        "0.75 HP": 13.8,
...
DATOS_SISTEMA_VOLTAJE = {
    "Monofásico 120V": {"v_ln": 120, "v_ll": 120},
    "220V Monofásico (L1 + L2)": {"v_ln": 110, "v_ll": 220},
    "Bifásico 120/208V": {"v_ln": 120, "v_ll": 208},
    "Trifásico 120/208V": {"v_ln": 120, "v_ll": 208},
    "Trifásico 277/480V": {"v_ln": 277, "v_ll": 480}
}
...
# --- FUNCIONES DE DESCLASIFICACIÓN Y EVALUACIÓN ---
...
# --- INICIALIZACIÓN DE TABLA VACÍA AL EMPEZAR ---
if "tabla_cargas" not in st.session_state:
    st.session_state.tabla_cargas = pd.DataFrame(columns=[
        "Descripción", "Cantidad", "Valor", "Unidad", "Fases", "FP"
    ])

# --- PANEL DE SELECCIÓN DE MOTORES NEC Y CARGAS ---
st.subheader("📋 Agregar Cargas y Motores Industriales")
...
with tab_motores:
    ...
    with col_tipo_m:
        tipo_motor_sel = st.selectbox("Configuración de Motor:", list(MOTORES_NEC.keys()))
    
    with col_hp_m:
        hp_sel = st.selectbox("Potencia Nominal (HP):", list(MOTORES_NEC[tipo_motor_sel].keys()))
    
    with col_cant_m:
        cant_m = st.number_input("Cantidad:", min_value=1, value=1, step=1, key="cant_motor_input")
...
with tab_cargas_gen:
    ...
    with col_sel_g:
        equipo_nuevo = st.selectbox("Librería de cargas generales:", list(EQUIPOS_PREDETERMINADOS.keys()))
    
    with col_cant_g:
        cant_nueva = st.number_input("Cantidad:", min_value=1, value=1, step=1, key="cant_gen_input")
...
# --- TABLA INTERACTIVA DE EDITAR/ELIMINAR ---
col_head, col_vaciar = st.columns([4, 1])
with col_head:
    st.subheader("📑 Cuadro de Cargas Consolidado")
with col_vaciar:
    if st.button("🗑️ Vaciar Tabla", use_container_width=True):
        st.session_state.tabla_cargas = pd.DataFrame(columns=["Descripción", "Cantidad", "Valor", "Unidad", "Fases", "FP"])
        st.rerun()

st.caption("📌 **Control de la tabla:** Haz doble clic en cualquier celda para modificar valores. Para eliminar filas individuales, selecciona la casilla izquierda y pulsa el icono de la papelera 🗑️.")

cargas_editadas = st.data_editor(
    st.session_state.tabla_cargas,
    num_rows="dynamic",
    column_config={
        "Descripción": st.column_config.TextColumn("Equipo / Motor / Circuito", help="Haz doble clic para cambiar el nombre"),
        "Cantidad": st.column_config.NumberColumn("Cantidad", min_value=1, step=1, default=1),
        "Valor": st.column_config.NumberColumn("Valor (Potencia o Amperios FLC)", min_value=0.01, step=0.1, default=1.0, format="%.2f"),
        "Unidad": st.column_config.SelectboxColumn("Unidad", options=["A", "kW", "kVA"], default="A"),
        "Fases": st.column_config.SelectboxColumn("Configuración de Fases", options=["Monofásica", "Bifásica", "Trifásica"], default="Monofásica"),
...
# --- CÁLCULO DE APARENTE TOTAL CON MATEMÁTICA NEC ---
total_kva_carga = 0.0
...
for _, row in cargas_editadas.iterrows():
    ...
    fases_carga = row.get("Fases", "Monofásica")
    fp = row.get("FP", 0.8)

    if unidad == "kVA":
        kva_unitario = val
    elif unidad == "kW":
        kva_unitario = (val / fp) if fp > 0 else val
    elif unidad == "A":
        ...
        elif fases_carga == "Bifásica":
            kva_unitario = (v_info["v_ll"] * val) / 1000.0
        elif fases_carga == "Trifásica":
            kva_unitario = (math.sqrt(3) * v_info["v_ll"] * val) / 1000.0
        else:
            kva_unitario = (v_info["v_ll"] * val) / 1000.0

    total_kva_carga += kva_unitario * cant
...
if total_kva_carga == 0.0:
    ...
    if aptos_filtrados:
        ...
        pct_opt = (total_kva_carga / cap_efectiva_opt) * 100 if cap_efectiva_opt > 0 else 0
        ...
        with col_exp:
            ...
            if idx_opt + 1 < len(cat):
                ...
                pct_exp = (total_kva_carga / cap_efectiva_exp) * 100 if cap_efectiva_exp > 0 else 0
    ...
    df_eval["Carga (%)"] = (total_kva_carga / df_eval["Capacidad Efectiva (kVA)"]) * 100 if total_kva_carga > 0 else 0.0
